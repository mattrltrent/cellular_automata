...
export const gridSlice = createSlice({
  ...
  initialState,
  reducers: {
    initGrid: (state, action: PayloadAction<{ rows: number, columns: number }>) => {
      const { rows, columns } = action.payload;
      state.data = Array.from({ length: rows }, () =>
        Array.from({ length: columns }, () => ({ isAlive: false }))
      );
    },
    updateCellState: (state, action: PayloadAction<{ rowIndex: number, columnIndex: number }>) => {
    ...
    tick: (state) => {
      const nextGrid = state.data.map((row, rowIndex) =>
        row.map((cell, columnIndex) => {
          const aliveNeighbors = countNeighbors(state.data, rowIndex, columnIndex);
